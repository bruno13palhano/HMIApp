...
internal class MqttClientRepositoryImpl @Inject constructor(
    ...
    override suspend fun connectMqtt(mqttConnectionConfig: MqttConnectionConfig): Result<Unit> =
        mqtt.connect(mqttConnectionConfig = mqttConnectionConfig).onSuccess {
    ...
    override suspend fun publish(topic: String, message: String): Result<Unit> =
        mqtt.publish(topic = topic, message = message)

    override fun isConnected(): Flow<Boolean> = mqtt.isConnected()

    override suspend fun disconnect(): Result<Unit> = mqtt.disconnect().onSuccess {
